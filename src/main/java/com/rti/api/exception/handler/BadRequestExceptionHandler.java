...
public class BadRequestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    @ExceptionHandler({})
    public final ResponseEntity<Response> handleBadRequestExceptions(CustomStandardException ex, WebRequest request) {
        log.error(((ServletWebRequest) request).getRequest().getRequestURI(), ex);
        return ResponseHandler.handleBadRequestResponse(ex.getCategory(), List.of(ex.getLocalizedMessage()));
    }
}
